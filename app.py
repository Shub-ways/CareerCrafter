...
# import shutil
# import base64
# import time  # Optional for potential logging
...
# # ---- Cached Helpers ----
# @st.cache_data
# def load_json_cached(_file):
#     try:
#         if os.path.exists(_file):
#             with open(_file, "r", encoding="utf-8") as f:
#                 content = f.read().strip()
#                 if not content:
#                     return {} if _file in [PROFILE_FILE, USERS_FILE, PROFILE_PICS_FILE] else []
...
# def get_profile_image_base64_cached(_path):
#     try:
...
# @st.cache_data
# def get_gemini_response_cached(user_input, roles_data=None):
#     return get_gemini_response(user_input, roles_data=roles_data)
...
#         elif key == "history":
#             all_history = load_json_cached(HISTORY_FILE)
#             current_user = st.session_state.get("username", "")
#             st.session_state[key] = all_history.get(current_user, [])
#         elif key == "profile_photo":
#             st.session_state[key] = DEFAULT_PROFILE_PIC
#         else:
#             st.session_state[key] = load_json_cached({
#                 "peers": PEERS_FILE,
...
# profile_pics = load_json_cached(PROFILE_PICS_FILE)
...
#         # Load profile
#         all_profiles = load_json_cached(PROFILE_FILE)
...
#         profile_img_b64 = get_profile_image_base64_cached(get_profile_image_path())
#         if profile_img_b64:
#             st.markdown(f"""
...
#                 # Removed st.rerun() to let Streamlit handle state change automatically
        
#         # Removed JS script for simplicity and performance
...
#                 response = get_gemini_response_cached(user_input, roles_data=user_data)
